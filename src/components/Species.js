...
import { SearchContext, W2VValueContext, RasterContext } from "../context/Context";
...
const Species = ({ mode }) => {
  ...
  const getSuggestionsGBIF = (q) => {
    const url_vernacular = `https://api.gbif.org/v1/species/search?q=${encodeURI(q)}&qField=VERNACULAR&status=ACCEPTED&datasetKey=d7dddbf4-2cf0-4f39-9b2a-bb099caae36c&limit=50` //&qField=VERNACULAR &habitat=freshwater/terrestrial/marine
    const url_canonical = `https://api.gbif.org/v1/species/suggest?q=${encodeURI(q)}&status=ACCEPTED&limit=50`
    fetch(url_canonical)
      .then(res => res.json())
      .then(data => {
