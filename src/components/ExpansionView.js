...
import { ExpansionContext, ExpansionModeContext, W2VValueContext } from '../context/Context';
...
const ExpansionView = () => {
    ...
    //    const G = { ...expansion, nodes: [{ ...value, score: 1 }], expanded: [value] }
    //    expand(G)
    //}
    ...
    useEffect(() => {
        ...
        if (value !== null) {
            var G = {}
            if (expansion.nodes.length === 0) {
                G = { ...expansion, nodes: [{ name: value, type: mode, score: 1 }], expanded: [value] }
            } else {
                G = { ...expansion, expansions: [] }
            }
            expand(G)
        }
    }, [value, mode])
